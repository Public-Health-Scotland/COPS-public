...
fetuslevel <- read_rds(paste0(folder_temp_data, archive_folder, "script6_baby_level_record_infection.rds")) 

fetuslevel <- fetuslevel %>% 
  mutate(positive_before_study_start = case_when(tests_mother_earliest_positive_test < delta_start ~1,
                                                 TRUE~0))
##################
fetus_infections <- fetuslevel %>% 
  filter(positive_before_study_start ==0) %>% # remove positive tests before start of study
  filter(tests_mother_earliest_positive_test==tests_mother_value_positive_test_during_pregnancy_1) %>%  ##only FIRST postive tests count
  filter(tests_mother_positive_test_during_pregnancy==1 & 
...
#pregnancies####
pregnancies <-readRDS(paste0(folder_temp_data, archive_folder, "script6b_pregnancy_level_record.rds"))
...
pregnancies_infections <- pregnancies %>% 
  filter(mother_earliest_positive_test==mother_positive_test_during_pregnancy_1) %>% 
...
saveRDS(demographics_summaries, paste0(folder_temp_data,  "variants_analysis/sensistivity_analysis/","temp_variant_demogs1_no_transition.rds"))
...
generate_recodes <- function(df_labelled, colname, output = "vector"){
 ...
 # attr(labelled::val_labels(df_ep$unit_outcome), "names")
...
saveRDS(pregnancies_icu_21, paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "icu_model_data.rds"))
model1 <- glm(flag_covid_associated ~ variant_period, data=pregnancies_icu_21, 
              family= binomial(link = "logit") )
summary(model1)
#plot(model1)
OR <- as.data.frame(exp(cbind(coef(model1), confint(model1))))
names(OR) <- c("OR", "LCI", "UCI")

saveRDS(OR,paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "icu_unadj_OR.rds" ))

model1 <- glm(flag_covid_associated ~ variant_period + simd + vaccination_status_at_infection  +
                gestation_at_infection+ + ethnicity_reporting+
...
saveRDS(OR,paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "icu_adj_OR.rds" ))
...
saveRDS(pregnancies_icu, paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "icu_diag_model_data.rds"))

model1 <- glm(flag_covid_diag_in_stay ~ variant_period, data=pregnancies_icu, 
                family= binomial(link = "logit") )
summary(model1)
OR <- as.data.frame(exp(cbind(coef(model1), confint(model1))))
names(OR) <- c("OR", "LCI", "UCI")
OR
saveRDS(OR,paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "icu_diag_unadj_OR.rds" ))

model1.1 <- glm(flag_covid_diag_in_stay ~ variant_period + simd + vaccination_status_at_infection  +
                  gestation_at_infection+ethnicity_reporting+
                mother_age_at_conception, data=pregnancies_icu, 
              family= binomial(link = "logit") )
summary(model1.1)
OR <- as.data.frame(exp(cbind(coef(model1.1), confint(model1.1))))
names(OR) <- c("OR", "LCI", "UCI")
OR
saveRDS(OR,paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "icu_diag_adj_OR.rds" ))
...
saveRDS(fetus_preterm_denom , paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "preterm_model_data.rds"))
...
saveRDS(summ(model2_1, exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "preterm_unadj_OR.rds" ))
...
saveRDS(summ(model2,exp=T, confint = T), paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "preterm_model_summary.rds" ))
...
saveRDS(summ(model2a, exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "preterm_model_summary_eth.rds" ))
...
saveRDS(all_births , paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "stillbirth_model_data.rds"))
...
model3_1 <-  svyglm(stillbirth~ variant_period,
                    ...
                    family= binomial(link = "logit") )
summ(model3_1,  exp=T, confint = T)
saveRDS(summ(model3_1,  exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "stillb_unadj_OR.rds" ))
...
model3 <-  svyglm(stillbirth~ variant_period + x_simd + vaccination_status_at_infection +gestation_at_infection 
                   ...
                   design = design, data=all_births, 
                   family= binomial(link = "logit") )
summ(model3,  exp=T, confint = T)


saveRDS(summ(model3,  exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "stillbirth_model_summary.rds" ))
...
saveRDS(all_live_births , paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "NND_model_data.rds"))
...
model4_1 <-  svyglm(NND~ variant_period,
                  ...
                  family= binomial(link = "logit") )
summ(model4_1,  exp=T, confint = T)
saveRDS(summ(model4_1,  exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "NND_unadj_OR.rds" ))
...
model4 <-  svyglm(NND~ variant_period + x_simd + vaccination_status_at_infection +gestation_at_infection  + x_mother_age_at_outcome+
                   ...
                   design = design, data=all_live_births, 
                   family= binomial(link = "logit") )
summ(model4,  exp=T, confint = T)
saveRDS(summ(model4,  exp=T, confint=T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "NND_model_summary_ethn.rds" ))

model4a <-  svyglm(NND~ variant_period + x_simd + vaccination_status_at_infection +gestation_at_infection  + x_mother_age_at_outcome,
                  design = design, data=all_live_births, 
                  family= binomial(link = "logit") )
summ(model4a,  exp=T, confint = T)
saveRDS(summ(model4a,  exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "NND_model_summary.rds" ))
...
table(term_births$low_apgar, term_births$variant_period)
table(term_births$low_apgar, term_births$ethnicity_reporting)

saveRDS(term_births  , paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "apgar_model_data.rds"))
...
model5_1 <-  svyglm(low_apgar~ variant_period,
                    ...
                    family= binomial(link = "logit") )
summ(model5_1, exp=T)
saveRDS(summ(model5_1, exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "apgar_unadj_OR.rds" ))
...
model5 <-  svyglm(low_apgar ~ variant_period + x_simd + vaccination_status_at_infection +
                  ...
                  family= binomial(link = "logit") )
summ(model5, exp=T)
saveRDS(summ(model5,  exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "apgar_model_summary_ethn.rds" ))
...
model5a <-  svyglm(low_apgar ~ variant_period + x_simd + vaccination_status_at_infection +
                  ...
                  family= binomial(link = "logit") )
summary(model5a)
saveRDS(summ(model5a, exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "apgar_model_summary.rds" ))
...
NNI_table
saveRDS(NNI_table, paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "NNI_table.rds"))

table(live_births$neonate_infection,live_births$variant_period, useNA="always")
table(live_births$neonate_infection,live_births$ethnicity_reporting, useNA="always")
saveRDS(live_births  , paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "neonate_infect_model_data.rds"))
...
saveRDS(summ(model6_1, exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "NNI_unadj_OR.rds" ))
...
saveRDS(summ(model6,  exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "NNI_model_summary.rds" ))
...
saveRDS(summ(model7_1, exp=T, confint = T),paste0(folder_temp_data, "variants_analysis/sensistivity_analysis/", "earlyNNI_unadj_OR.rds" ))
