#################################################
###### BABY OUTCOMES MODELS - INFECTIONS ######
#################################################
...
# -	<30 events: no adjustment
# -	30-89 events: adjustment for parity and deprivation only
# -	90+ events: adjustment for all eight covariates
...
table(cohort1_contemp$perinatal_outcome, useNA = "ifany")t

addmargins(table(cohort1_contemp$parity_cat, cohort1_contemp$inf_or_uninf, cohort1_contemp$stillbirth_outcome))
...
model1b <- clogit(stillbirth_outcome ~ exposure  + simd + ethnicity_cat + parity_cat +
...
addmargins(table(cohort2_contemp$parity_cat, cohort2_contemp$inf_or_uninf, cohort2_contemp$nnd_outcome))
...
## FULLY ADJUSTED MODEL ##
model12a <- clogit(spontaneous_very_preterm_outcome ~ exposure + simd  + 
                   + strata(index), 
...
## FULLY ADJUSTED MODEL ##
model13a <- clogit(provider_initiated_very_preterm_outcome ~ exposure + simd + strata(index), 
                  data=cohort4_contemp_deliveryonset)
summary(model13a)
