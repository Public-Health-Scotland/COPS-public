##################################################################################################
###### BABY OUTCOMES MODELS - SUPPLEMENTARY ANALYSIS 2: INFECTIONS AFTER 20 WEEKS ######
##################################################################################################
...
filter_post_20_week_gestation <- function(data, cohort=1) {
  
  if(cohort == 3) {
    
    data <- data %>%
      mutate(gestation_at_infection = ptb_gestation_at_index_date)

    
    } else if(cohort == 4) {
    
    data <- data %>%
      mutate(gestation_at_infection = vptb_gestation_at_index_date)


    } else {
    
    data <- data %>%
      mutate(gestation_at_infection = perinatal_gestation_at_index_date)
    
    }
  
  filtered_data <- data %>% 
    filter(gestation_at_infection >= 20)
...
model1a <- clogit(stillbirth_outcome ~ exposure + simd  + parity_cat + strata(index), 
...
## FULLY ADJUSTED MODEL ##
model2a <- clogit(perinatal_outcome ~ exposure + simd  + parity_cat + strata(index), 
                  data=cohort1_contemp)
summary(model2a)
...
cohort4_contemp <- cohort4_infect_contemp_controls_singletons %>%
  filter_post_20_week_gestation(., cohort = 4) %>%
...
## FULLY ADJUSTED MODEL ##
model8a <- clogit(very_preterm_outcome ~ exposure + simd + strata(index), 
...
## FULLY ADJUSTED MODEL ##
model10a <- clogit(provider_initiated_very_preterm_outcome ~ exposure + simd + ethnicity_cat  +
                     UR2_categories + cv_clinical_vulnerability_category + diabetes_cat_2 + bmi_cat + smoking_status + strata(index), 
                   data=cohort4_contemp_deliveryonset)
summary(model10a)
...
## FULLY ADJUSTED MODEL ##
model13a <- clogit(very_sga_outcome ~ exposure + simd + ethnicity_cat + parity_cat +
                     UR2_categories + cv_clinical_vulnerability_category + diabetes_cat_2 + bmi_cat + smoking_status + strata(index), 
                   data=cohort5_contemp)
summary(model13a)
...
saveRDS(cohort1_contemp, paste0(folder_results, "post20weeks_stillbirth_peri.rds"))
saveRDS(cohort2_contemp, paste0(folder_results, "post20weeks_nnd.rds"))
saveRDS(cohort3_contemp, paste0(folder_results, "post20weeks_preterm.rds"))
saveRDS(cohort3_contemp_deliveryonset, paste0(folder_results, "post20weeks_preterm_deliveryonset.rds"))
saveRDS(cohort4_contemp, paste0(folder_results, "post20weeks_very_preterm.rds"))
saveRDS(cohort4_contemp_deliveryonset, paste0(folder_results, "post20weeks_very_preterm_deliveryonset.rds"))
saveRDS(cohort5_contemp, paste0(folder_results, "post20weeks_sga.rds"))
saveRDS(cohort6_contemp, paste0(folder_results, "post20weeks_apgar.rds"))

# save model output for forest plot

saveRDS(model1, paste0(folder_results, "models/subgroup_analysis_20weeks_stillbirth_unadjusted.rds"))
saveRDS(model1a, paste0(folder_results, "models/subgroup_analysis_20weeks_stillbirth_adjusted.rds"))

saveRDS(model2, paste0(folder_results, "models/subgroup_analysis_20weeks_perinatal_mortality_unadjusted.rds"))
saveRDS(model2a, paste0(folder_results, "models/subgroup_analysis_20weeks_perinatal_mortality_adjusted.rds"))

saveRDS(model3, paste0(folder_results, "models/subgroup_analysis_20weeks_nnd_unadjusted.rds"))

saveRDS(model4, paste0(folder_results, "models/subgroup_analysis_20weeks_preterm_unadjusted.rds"))
saveRDS(model4a, paste0(folder_results, "models/subgroup_analysis_20weeks_preterm_adjusted.rds"))

saveRDS(model5, paste0(folder_results, "models/subgroup_analysis_20weeks_spontaneous_preterm_unadjusted.rds"))
saveRDS(model5a, paste0(folder_results, "models/subgroup_analysis_20weeks_spontaneous_preterm_adjusted.rds"))

saveRDS(model6, paste0(folder_results, "models/subgroup_analysis_20weeks_provider_initiated_preterm_unadjusted.rds"))
saveRDS(model6a, paste0(folder_results, "models/subgroup_analysis_20weeks_provider_initiated_preterm_adjusted.rds"))

saveRDS(model8, paste0(folder_results, "models/subgroup_analysis_20weeks_very_preterm_unadjusted.rds"))
saveRDS(model8a, paste0(folder_results, "models/subgroup_analysis_20weeks_very_preterm_adjusted.rds"))

saveRDS(model9, paste0(folder_results, "models/subgroup_analysis_20weeks_spontaneous_very_preterm_unadjusted.rds"))
saveRDS(model9a, paste0(folder_results, "models/subgroup_analysis_20weeks_spontaneous_very_preterm_adjusted.rds"))

saveRDS(model10, paste0(folder_results, "models/subgroup_analysis_20weeks_provider_initiated_very_preterm_unadjusted.rds"))
saveRDS(model10a, paste0(folder_results, "models/subgroup_analysis_20weeks_provider_initiated_very_preterm_adjusted.rds"))

saveRDS(model12, paste0(folder_results, "models/subgroup_analysis_20weeks_small_gestational_age_unadjusted.rds"))
saveRDS(model12a, paste0(folder_results, "models/subgroup_analysis_20weeks_small_gestational_age_adjusted.rds"))

saveRDS(model13, paste0(folder_results, "models/subgroup_analysis_20weeks_very_small_gestational_age_unadjusted.rds"))
saveRDS(model13a, paste0(folder_results, "models/subgroup_analysis_20weeks_very_small_gestational_age_adjusted.rds"))

saveRDS(model14, paste0(folder_results, "models/subgroup_analysis_20weeks_low_apgar_unadjusted.rds"))
saveRDS(model14a, paste0(folder_results, "models/subgroup_analysis_20weeks_low_apgar_adjusted.rds"))

saveRDS(model15, paste0(folder_results, "models/subgroup_analysis_20weeks_very_low_apgar_unadjusted.rds"))
saveRDS(model15a, paste0(folder_results, "models/subgroup_analysis_20weeks_very_low_apgar_adjusted.rds"))
