...
knitr::opts_chunk$set(echo = FALSE)
  ...
  publication_latest_vacc_date <- lubridate::ymd("2022-04-30") 
...
number_positive_neonates_apr22 <- readRDS(paste0(folder_temp_data, "number_neonatal_infections_testing_only_apr22.rds"))
...
baby_level <- readRDS(paste0(folder_temp_data, "script6_baby_level_record_infection.rds")) %>% filter(x_full_cohort == T) %>%
  ...
  filter(x_est_conception_date <= as.Date("2022-04-30"))
...
##edit for August 2022 update only - limit everything in this rmd to end april
numbers_base <- baby_level %>% filter(x_est_conception_date <= as.Date("2022-04-30")) %>% 
  select(pregnancy_id, mother_upi, baby_upi, x_pregnancy_end_date, outcome, chi_validity, baby_chi_validity, x_est_conception_date) %>% 
...
n_comp_preg_valid_chi <- numbers_base %>%
  ...
  filter(x_pregnancy_end_date <= as.Date("2022-04-30")) %>% 
  group_by(pregnancy_id) %>% 
  slice(1) %>%
  nrow()     

n_ongoing_preg_valid_chi <- numbers_base %>%
  filter(outcome == "Ongoing" | x_pregnancy_end_date > as.Date("2022-04-30")) %>% 
  group_by(pregnancy_id) %>% 
  slice(1) %>%
  nrow()     
...
preg_infections_total <- pregnancies  %>%
  ...
  filter(mother_earliest_positive_test_during_pregnancy <= as.Date("2022-04-30")) %>%
...
From mid April https://www.gov.scot/publications/test-protect-transition-plan/ asymptomatic individuals were no longer encouraged to test, except in certain circumstance; and from May 2022 onwards, testing was limited mainly to healthcare settings. For this reason, this final report on infections in pregnancy only reports on case numbers to the end of April 2022. 
